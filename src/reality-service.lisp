...
(defun reality-routes (actor)
  (labels ((state-json (fn)
    ...
    (list
     ...
     ;; so the default changes nothing: the audit trail is evidence, and a
     ;; rewind of run state is not a reason to discard it.
     ;;
     ;; Read from the query string or the JSON body, because the resets are
     ;; called both ways across the harness and a caller should not have to know
     ;; which.
                 ...
                 (lambda (_ body query)
                   ...
                   (let* ((q (and query (jget query "clearAudit")))
                     ...
                     (state-json (lambda (state)
                                   ...
                                   (obj "success" t
                                        "auditCleared" (if clear-audit t :false)))))))
