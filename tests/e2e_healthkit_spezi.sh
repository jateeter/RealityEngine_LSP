...
[ -x bin/reality-engine-lsp ] || { echo "bin/reality-engine-lsp missing; run make build first" >&2; exit 1; }

echo "HealthKit Spezi bridge e2e (LSP)"
...
PERCEPTION_ENGINE_PORT="$PERCEPTION_ENGINE_E2E_PORT" \
  ...
  bin/reality-engine-lsp perception >/tmp/perception_engine_healthkit_spezi_lsp_e2e.log 2>&1 &
PERCEPTION_PID="$!"
